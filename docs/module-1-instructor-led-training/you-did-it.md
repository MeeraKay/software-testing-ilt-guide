# You did it!

## End of Instructor-Led Training

**Congratulations!** You have completed the instructor-led portion of this course.

To reinforce your learning and qualify for course completion, proceed with the following sections:

- **[Module 2 — Course Completion Exercises](../module-2-course-completion-exercises/index.md)** — complete and submit the required exercises.
- **[Module 3 — Additional Topics](../module-3-additional-topics/index.md)** — explore optional topics to deepen your understanding and extend your skills.

---

[Next → Module 2: Course Completion Exercises](../module-2-course-completion-exercises/index.md){: .md-button .md-button--primary}

---
